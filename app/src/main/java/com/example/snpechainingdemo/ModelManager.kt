...
//    private val modelDir = context.getExternalFilesDir(null)!! // Private external storage
    ...
    suspend fun syncAndGetPaths(onProgress: ProgressListener): Map<String, String>? {
        ...
        try {
            ...
            // 1. GLOBAL GATEKEEPER
            val remoteRepoSha = fetchRepoSha() //?: return null
...
//            }
            if ((remoteRepoSha != null && remoteRepoSha == lastSyncedSha) || (remoteRepoSha == null)) {
                ...
                if (localManifest != null && !localManifest.required_models.isNullOrEmpty()) {
                    Log.d("ModelSync", "Verifying local assets...")
                    onProgress(if (remoteRepoSha == null) "Offline mode. Verifying local assets..." else "Verifying local assets...")
                    ...
                    for (model in localManifest.required_models) {
                        ...
                        if (file.exists() && cachedHash.isNotEmpty()) {
                            ...
                            Log.w("ModelSync", "Fast Path failed: ${model.file} missing or unverified.")
                            allLocallyValid = false
                            break // Stop checking, we need a full sync
                        }
                    }
                    if (allLocallyValid) {
                        Log.d("ModelSync", "System up to date and verified.")
                        onProgress("Assets verified.")
                        return fastPathMap
                    }
                }
            }
            ...
            if (remoteRepoSha == null) {
                onProgress("Offline.")
                Log.d("ModelSync", "It seems you are offline: remoteRepoSha is null. Will not proceed with fetching manifest or model downloads.")
                return null
            }
            ...
            val manifest = fetchRichManifest() ?: throw IOException("Manifest missing")
            if (manifest.required_models.isNullOrEmpty()) {
                Log.e("ModelSync", "Manifest parsed but 'models' list is empty or null!")
                throw IOException("Manifest empty")
//                return null
            }
            // SAVE MANIFEST FOR NEXT TIME
            saveManifestLocally(manifest)

            // Optional: We could check manifest.suite_info.min_app_version here

            val treeItems = fetchTreeMetadata() ?: throw IOException("Metadata missing")
            ...
            for (modelConfig in manifest.required_models) {
                ...
                try {
                    ...
                    val remoteInfo = treeItems.find { it.path == filename } ?: throw IOException("Remote metadata not found for $filename")
                    val expectedHash = remoteInfo.lfs?.oid ?: throw IOException("No LFS hash found for $filename")
                    ...
                    if (!isLocallyValid(targetFile, expectedHash)) {
                        ...
                        downloadAndVerify(filename, targetFile, expectedHash, onProgress)
                        ...
                        Log.d("ModelSync", "Locally valid file found for ${modelConfig.id} ($filename)...")
                        validPaths[filename] = targetFile.absolutePath
                    }
                } catch (e: Exception) {
                    Log.e("ModelSync", "Failed to sync $filename: ${e.message}")
            ...
            if (errors.isNotEmpty()) {
                onProgress("Sync finished with ${errors.size} errors.\n PLEASE RESTART APP!")
                Log.e("ModelSync", "Errors encountered: $errors")
...
//                return getLocalPaths()
    ...
    private fun downloadAndVerify(filename: String, destination: File, expectedHash: String, onProgress: ProgressListener) {
        ...
        client.newCall(request).execute().use { response ->
            if (!response.isSuccessful) throw IOException("Download failed: $filename")
...
//            }
            ...
            // 4. Success
            prefs.edit().putString("hash_$filename", calculatedHash).apply()
...
//        prefs.edit().putString("hash_$filename", downloadedHash).apply()
    ...
    private fun fetchRepoSha(): String? {
        val req = Request.Builder().url("https://huggingface.co/api/models/$repo").header("Authorization", "Bearer $hfToken").build()
        val json = client.newCall(req).execute().body?.string() ?: return null
        return Gson().fromJson(json, HFRepoInfo::class.java).sha
    }

    private fun fetchManifest(): SimpleManifest? {
        val req = Request.Builder().url("https://huggingface.co/$repo/resolve/main/manifest.json").header("Authorization", "Bearer $hfToken").build()
        return Gson().fromJson(client.newCall(req).execute().body?.charStream(), SimpleManifest::class.java)
    }

    private fun fetchRichManifest(): NeuroSymbolicManifest? {
        val req = Request.Builder()
            .url("https://huggingface.co/$repo/resolve/main/manifest.json")
            .header("Authorization", "Bearer $hfToken")
            .build()
    ...
    private fun fetchTreeMetadata(): List<HFTreeItem>? {
        val req = Request.Builder().url("https://huggingface.co/api/models/$repo/tree/main").header("Authorization", "Bearer $hfToken").build()
