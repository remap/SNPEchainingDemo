...
#ifndef SNPECHAININGDEMO_SDXLPIPELINEGRANULAR_H
#define SNPECHAININGDEMO_SDXLPIPELINEGRANULAR_H
...
struct Unet {
    ...
    std::unique_ptr<GraphRunner> unet_db1;
    std::unique_ptr<GraphRunner> unet_db2;
    std::unique_ptr<GraphRunner> unet_mb;
    std::unique_ptr<GraphRunner> unet_upblock00;
    std::unique_ptr<GraphRunner> unet_upblock01;
    std::unique_ptr<GraphRunner> unet_upblock02;
    std::unique_ptr<GraphRunner> unet_upblock1;
    std::unique_ptr<GraphRunner> outConv;
};

class SDXLPipelineGranular {
public:

    SDXLPipelineGranular(TensorWorkspace& ws,
               GraphRunner& enc_gr,
               AAssetManager* mgr,
        ...
        unet_.unet_db1.reset(new GraphRunner(ws_));
        unet_.unet_db2.reset(new GraphRunner(ws_));
        unet_.unet_mb.reset(new GraphRunner(ws_));
        unet_.unet_upblock00.reset(new GraphRunner(ws_));
        unet_.unet_upblock01.reset(new GraphRunner(ws_));
        unet_.unet_upblock02.reset(new GraphRunner(ws_));
        unet_.unet_upblock1.reset(new GraphRunner(ws_));
    ...
    SDXLPipelineGranular(TensorWorkspace& ws,
        ...
        unet_.time_embd.reset(new GraphRunner(ws));
        unet_.aug_emb.reset(new GraphRunner(ws));
        unet_.inConv.reset(new GraphRunner(ws));
        unet_.unet_db1.reset(new GraphRunner(ws));
        unet_.unet_db2.reset(new GraphRunner(ws));
        unet_.unet_mb.reset(new GraphRunner(ws));
        unet_.unet_upblock00.reset(new GraphRunner(ws));
        unet_.unet_upblock01.reset(new GraphRunner(ws));
        unet_.unet_upblock02.reset(new GraphRunner(ws));
        unet_.unet_upblock1.reset(new GraphRunner(ws));
        unet_.outConv.reset(new GraphRunner(ws));

        vae_decoder_.reset(new GraphRunner(ws));
    };
...
#endif //SNPECHAININGDEMO_SDXLPIPELINEGRANULAR_H